...
def autogenerate_code():
  ks = ["x", "y", "jitter_radius", "jitter_speed", "size", "filter", "filter2", "x0", "y0", "foreground", "paper_id"]
  times = ["", "last_"]
  print("""
...
*/
  ...
  uniform float u_paper_id_buffer_num;
  uniform float u_paper_id_constant;
  uniform float u_paper_id_transform;
  uniform vec2 u_paper_id_domain;
  uniform vec2 u_paper_id_range;
  uniform float u_paper_id_map_position;
  float a_paper_id;
  bool a_paper_id_is_constant;


  uniform float u_last_paper_id_buffer_num;
  uniform float u_last_paper_id_constant;
  uniform float u_last_paper_id_transform;
  uniform vec2 u_last_paper_id_domain;
  uniform vec2 u_last_paper_id_range;
  uniform float u_last_paper_id_map_position;
  float a_last_paper_id;
  bool a_last_paper_id_is_constant;
...
// ------------------------------------------------    
  ...
  if (u_paper_id_buffer_num > -0.5) {
    a_paper_id = get_buffer(u_paper_id_buffer_num);
    a_paper_id_is_constant = false;
  } else {
    a_paper_id = ix;
    a_paper_id_is_constant = true;
  }

  if (u_last_paper_id_buffer_num > -0.5) {
    a_last_paper_id = get_buffer(u_last_paper_id_buffer_num);
    a_last_paper_id_is_constant = false;
  } else {
    a_last_paper_id = ix;
    a_last_paper_id_is_constant = true;
  }
 ...
 for (int i = 0; i < MAX_PAPER_IDS; i++) {
    ...
    if (abs(stored_id - a_paper_id) < 0.1) {
...
/*  position = vec2(
  ...
  if (filter2_status < .5 || (ease < ix_to_random(ix, 1.) && last_filter2_status < .5)) {
    gl_Position = discard_me;
    return;
  }
...
/*  if (u_use_glyphset > 0. && point_size > 5.0) {
  ...
  if(is_paper_id_in_list) {
    // Scale based on current size - larger enhancement for smaller points,
    // more moderate enhancement for already large points
    float base_size = gl_PointSize;
    float scale_factor = min(4.0, max(2.0, 8.0 - base_size * 0.3));
    gl_PointSize *= scale_factor;
    
    // Store the original point size for use in fragment shader
